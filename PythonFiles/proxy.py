...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # * Fill in start 
        proxy_socket.bind((proxy_host,proxy_port)) # bind the socket to the proxy_host and proxy_port
        proxy_socket.listen() # listen for incoming connections
        # * Fill in end (1)
        ...
        while True:
            try:
                ...
                client_socket, client_address = proxy_socket.accept() # accept the connection
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int],
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            data = client_socket.recv(8192) #  # recieve data from the client 
            if not data:
                break
            try:
                # Process the request
                ...
                # * Fill in start(4)
                client_socket.sendall(response) # send the response
                # * Fill in end (4)
